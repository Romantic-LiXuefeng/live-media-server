...
namespace _srs_internal
{
    ...
    */
    ...
    int SrsDH::initialize(bool ensure_128bytes_public_key)
    {
        ...
        for (;;) {
            ...
            if (ensure_128bytes_public_key) {
                dint32 key_size = BN_num_bytes(pdh->pub_key);
                if (key_size != 128) {
    ...
    int SrsDH::copy_public_key(char* pkey, dint32& pkey_size)
    {
        int ret = ERROR_SUCCESS;
    ...
    int SrsDH::copy_shared_key(const char* ppkey, dint32 ppkey_size, char* skey, dint32& skey_size)
    {
        int ret = ERROR_SUCCESS;
        ...
        dint32 key_size = DH_compute_key((unsigned char*)skey, ppk, pdh);
    ...
    int key_block::parse(DStream stream)
    {
        ...
        int valid_offset = calc_valid_offset();
        if (valid_offset < 0) {
            return ERROR_KEY_BLOCK_INVALID_OFFSET;
        }
    ...
    int digest_block::parse(DStream stream)
    {
        ...
        int valid_offset = calc_valid_offset();
        if (valid_offset < 0) {
            return ERROR_DIGEST_BLOCK_INVALID_OFFSET;
        }
    ...
    int c1s1_strategy_schema0::copy_to(c1s1 *owner, char *bytes, int size, bool with_digest)
    {
        ...
        memcpy(bytes, stream.data(), stream.size());
...
int rtmp_handshake::handshake_with_client()
{
    ...
    if ((ret = complex_handshake_with_client()) != ERROR_SUCCESS) {
        if (ret == ERROR_HANDSHAKE_TRY_SIMPLE_HS) {
            m_complex = false;
            return simple_handshake_with_client();
        }
    }
...
int rtmp_handshake::handshake_with_server(bool complex)
{
    int ret = ERROR_SUCCESS;

    m_complex = complex;

    if ((ret = complex_handshake_with_server()) != ERROR_SUCCESS) {
        if (ret == ERROR_HANDSHAKE_TRY_SIMPLE_HS) {
            m_complex = false;
            return simple_handshake_with_server();
        }
    }
...
int rtmp_handshake::simple_handshake_with_client()
{
    int ret = ERROR_SUCCESS;
    ...
    if (!m_handshaking) {
        ...
        create_s0s1s2(c0c1_data + 1);

        SendBuffer *buffer = new SendBuffer;
        buffer->chunk = s0s1s2;
        buffer->pos = 0;
        buffer->len = s0s1s2->length;

        m_socket->addData(buffer);

        m_handshaking++;
    }

    if (m_handshaking == 1) {
        if ((ret = read_c2()) != ERROR_SUCCESS) {
            return ret;
        }

        log_trace("simple handshake with client success");
...
int rtmp_handshake::complex_handshake_with_client()
{
    int ret = ERROR_SUCCESS;

    if (!m_complex) {
        return ERROR_HANDSHAKE_TRY_SIMPLE_HS;
    }

    if (!m_handshaking) {
        if ((ret = read_c0c1()) != ERROR_SUCCESS) {
            return ret;
        }
        ...
        if ((ret = c1.c1_validate_digest(is_valid)) != ERROR_SUCCESS || !is_valid) {
            ...
            if ((ret = c1.c1_validate_digest(is_valid)) != ERROR_SUCCESS || !is_valid) {
                ret = ERROR_HANDSHAKE_TRY_SIMPLE_HS;
    ...
    if (m_handshaking == 1) {
        ...
        c2s2 cs;
        if ((ret = cs.parse(c2->data, 1536)) != ERROR_SUCCESS) {
            return ret;
        }
        ...
        log_trace("complex handshake with client success");
...
int rtmp_handshake::read_c0c1()
{
    ...
    if (m_socket->readData(c0c1->data, 1537) < 0) {
        return ERROR_HANDSHAKE_READ_C0C1;
    }

    return ret;
}

void rtmp_handshake::create_s0s1s2(const char *c1)
{
    if (s0s1s2.get()) {
        return;
    }
    ...
    memcpy(s0s1s2->data, stream.data(), 9);

    // if c1 specified, copy c1 to s2.
    if (c1) {
        memcpy(s0s1s2->data + 1537, c1, 1536);
    }
}

int rtmp_handshake::read_c2()
{
    int ret = ERROR_SUCCESS;
    ...
    if (m_socket->readData(c2->data, 1536) < 0) {
        return ERROR_HANDSHAKE_READ_C2;
    }

    return ret;
}

int rtmp_handshake::simple_handshake_with_server()
{
    int ret = ERROR_SUCCESS;
    ...
    if (m_handshaking == 1) {
        if ((ret = read_s0s1s2()) != ERROR_SUCCESS) {
            return ret;
        }
        ...
        if (s0s1s2_data[0] != 0x03) {
            ret = ERROR_HANDSHAKE_PLAIN_REQUIRED;
            log_warn("handshake failed, plain text required. ret=%d, %c", ret, s0s1s2_data[0]);
            return ret;
        }
...
int rtmp_handshake::complex_handshake_with_server()
{
    int ret = ERROR_SUCCESS;
    ...
    if (!m_handshaking) {
        ...
        if ((ret = m_c1->c1_create(srs_schema1)) != ERROR_SUCCESS) {
            return ret;
        }
        if ((ret = m_c1->dump(c0c1->data + 1, 1536)) != ERROR_SUCCESS) {
            return ret;
        }

        // verify c1
        bool is_valid;
        if ((ret = m_c1->c1_validate_digest(is_valid)) != ERROR_SUCCESS || !is_valid) {
            ret = ERROR_HANDSHAKE_TRY_SIMPLE_HS;
            return ret;
        }
    ...
    if (m_handshaking == 1) {       
        ...
        // verify s1s2
        c1s1 s1;
        if ((ret = s1.parse(s0s1s2_data + 1, 1536, m_c1->schema())) != ERROR_SUCCESS) {
            return ret;
        }
        ...
        c2s2 cs;
        if ((ret = cs.c2_create(&s1)) != ERROR_SUCCESS) {
            return ret;
        }

        if ((ret = cs.dump(c2->data, 1536)) != ERROR_SUCCESS) {
            return ret;
        }
...
int rtmp_handshake::read_s0s1s2()
{
    int ret = ERROR_SUCCESS;
    ...
    if (m_socket->readData(s0s1s2->data, 3073) < 0) {
        return ERROR_HANDSHAKE_READ_S0S1S2;
    }
