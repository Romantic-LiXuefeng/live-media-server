...
namespace _srs_internal
{
    ...
    */
    ...
    public:
        ...
        */
        virtual int copy_public_key(char* pkey, dint32 &pkey_size);
        /**
        ...
        *       user should never ignore this size.
        */
        virtual int copy_shared_key(const char* ppkey, dint32 ppkey_size, char* skey, dint32 &skey_size);
    private:
    ...
    */
    ...
    public:
        ...
        */
        virtual void copy_time_version(DStream &stream, c1s1* owner);
        /**
        * copy key to stream.
        */
        virtual void copy_key(DStream &stream);
        /**
        * copy digest to stream.
        */
        virtual void copy_digest(DStream &stream, bool with_digest);
    };
...
{
public:
    rtmp_handshake(DTcpSocket *socket);
    ~rtmp_handshake();

public:
    /**
     * @brief 握手成功返回0，握手中返回ERROR_HANDSHAKE_CONTINUE，其他为握手错误
     */
    int handshake_with_client();
    /**
     * @brief 握手成功返回0，握手中返回ERROR_HANDSHAKE_CONTINUE，其他为握手错误，默认简单握手
     */
    int handshake_with_server(bool complex = false);

private:
    int simple_handshake_with_client();
    int complex_handshake_with_client();

    int read_c0c1();
    void create_s0s1s2(const char *c1 = NULL);
    int read_c2();
...
private:
    // [1+1536]
    DSharedPtr<MemoryChunk> c0c1;
    // [1+1536+1536]
    DSharedPtr<MemoryChunk> s0s1s2;
    // [1536]
    DSharedPtr<MemoryChunk> c2;
